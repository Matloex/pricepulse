...
def get_price_trendyol(url: str):
    try:
        ...
        log.info(f"  Trendyol HTTP {r.status_code} [{url[:50]}]")
        if r.status_code != 200:
            log.warning(f"  Trendyol erişim engeli: {r.status_code}")
            return None, True
        ...
        if p:
            return p, in_stock
        for sel in [
            ".prc-dsc", ".pr-bx-pr-dsc span", ".product-price-container span",
            ".prc-slg", "[class*='prc-dsc']", "[class*='product-price']",
        ]:
            ...
            if el:
                ...
                if p:
                    return p, in_stock
        # Son çare: TL içeren span'ları tara
        for span in soup.find_all("span"):
            ...
            if "TL" in txt or "₺" in txt:
                ...
                if p and 10 < p < 1_000_000:
                    return p, in_stock
        log.warning(f"  Trendyol fiyat bulunamadı | başlık: {soup.title.string[:50] if soup.title else 'yok'}")
    except Exception as e:
...
def get_price_hepsiburada(url: str):
    try:
        r = SESSION.get(url, timeout=15)
        log.info(f"  Hepsiburada HTTP {r.status_code} [{url[:50]}]")
        if r.status_code != 200:
            log.warning(f"  Hepsiburada erişim engeli: {r.status_code}")
            return None, True
        soup = BeautifulSoup(r.content, "html.parser")
        ...
        if p:
            return p, in_stock
        for sel in [
            ".price-value", "[class*='currentPrice']", ".product-price",
            "[class*='price-value']", "[data-bind*='price']",
        ]:
            el = soup.select_one(sel)
            if el:
                p = parse_price(el.get_text())
                if p and 10 < p < 1_000_000:
                    return p, in_stock
        log.warning(f"  Hepsiburada fiyat bulunamadı | başlık: {soup.title.string[:50] if soup.title else 'yok'}")
    except Exception as e:
...
def get_price_amazon(url: str):
    try:
        ...
        if r.status_code != 200:
            log.warning(f"  Amazon erişim engeli: {r.status_code}")
            return None, True
        soup = BeautifulSoup(r.content, "html.parser")
        ...
        if p:
            return p, in_stock
        log.warning(f"  Amazon fiyat bulunamadı | başlık: {soup.title.string[:50] if soup.title else 'yok'}")
    except Exception as e:
...
def get_price_n11(url: str):
    try:
        ...
        if r.status_code != 200:
            log.warning(f"  N11 erişim engeli: {r.status_code}")
            return None, True
        soup = BeautifulSoup(r.content, "html.parser")
        for sel in [
            ".newPrice ins", ".price ins", ".prodPrice",
            ".fiyat", '[itemprop="price"]',
        ]:
            el = soup.select_one(sel)
            if el:
                p = parse_price(el.get("content") or el.get_text())
                if p and 10 < p < 1_000_000:
                    return p, True
        log.warning(f"  N11 fiyat bulunamadı | başlık: {soup.title.string[:50] if soup.title else 'yok'}")
    except Exception as e:
