...
import asyncio, logging, schedule, threading, time, json, os, copy
...
def is_silent():
    if not SETTINGS.get("silent_mode"): return False
    now = datetime.now()
...
def price_trend(product):
    ...
    if len(h) < 2: return "📊 Yeni takipte"
    prices = [x["price"] for x in h[-5:]]
    if prices[-1] < prices[0]:   return f"📉 {fmt(prices[0]-prices[-1])} düştü"
    elif prices[-1] > prices[0]: return f"📈 {fmt(prices[-1]-prices[0])} yükseldi"
    return "➡️ Stabil"
    ...
    try:
        ...
        for sc in soup.find_all("script", type="application/ld+json"):
            try:
                ...
                if "OutOfStock" in offers.get("availability", ""): in_stock = False
                price = offers.get("price")
                if price: return float(price), in_stock
            except: pass
        el = soup.select_one(".prc-dsc, .pr-bx-pr-dsc span")
        if el:
            raw = el.text.strip().replace(".", "").replace(",", ".").replace("TL", "").strip()
            return float(raw), in_stock
    except Exception as e: log.warning(f"Trendyol link hata: {e}")
    return None, True
...
def get_price_hepsiburada(url):
    ...
    except Exception as e: log.warning(f"Hepsiburada link hata: {e}")
    return None, True
...
def get_price_amazon(url):
    try:
        ...
        if whole:
            ...
            if frac: price += float(frac.text.strip()) / 100
            return price, in_stock
    except Exception as e: log.warning(f"Amazon link hata: {e}")
    return None, True
...
def get_price_n11(url):
    ...
    except Exception as e: log.warning(f"N11 link hata: {e}")
    return None, True

def fetch_price_link(product):
    p = product["platform"].lower()
    u = product["url"]
    if "trendyol"    in p: return get_price_trendyol(u)
    if "hepsiburada" in p: return get_price_hepsiburada(u)
    if "amazon"      in p: return get_price_amazon(u)
    if "n11"         in p: return get_price_n11(u)
    return None, True
...
def search_trendyol(query: str, limit: int = 5):
    ...
    except Exception as e: log.warning(f"Trendyol arama hata: {e}")
    return results
...
def search_hepsiburada(query: str, limit: int = 5):
    ...
    except Exception as e: log.warning(f"Hepsiburada arama hata: {e}")
    return results
...
def search_amazon(query: str, limit: int = 5):
    ...
    try:
        ...
        for card in cards:
            try:
                ...
                if not name_el or not whole_el: continue
                name  = name_el.get_text(strip=True)
    ...
    except Exception as e: log.warning(f"Amazon arama hata: {e}")
    return results
...
def search_n11(query: str, limit: int = 5):
    ...
    except Exception as e: log.warning(f"N11 arama hata: {e}")
    return results

def do_keyword_search(platform: str, query: str, limit: int = 5):
    p = platform.lower()
    if "trendyol"    in p: return search_trendyol(query, limit)
    if "hepsiburada" in p: return search_hepsiburada(query, limit)
    if "amazon"      in p: return search_amazon(query, limit)
    if "n11"         in p: return search_n11(query, limit)
    return []
...
def scrape_seller_trendyol(seller_url: str, limit: int = 10):
    ...
    except Exception as e: log.warning(f"Trendyol satıcı hata: {e}")
    return results
...
def scrape_seller_hepsiburada(seller_url: str, limit: int = 10):
    ...
    except Exception as e: log.warning(f"Hepsiburada satıcı hata: {e}")
    return results
...
def scrape_seller_amazon(seller_url: str, limit: int = 10):
    ...
    except Exception as e: log.warning(f"Amazon satıcı hata: {e}")
    return results

def do_seller_scrape(platform: str, seller_url: str, limit: int = 10):
    p = platform.lower()
    if "trendyol"    in p: return scrape_seller_trendyol(seller_url, limit)
    if "hepsiburada" in p: return scrape_seller_hepsiburada(seller_url, limit)
    if "amazon"      in p: return scrape_seller_amazon(seller_url, limit)
    return []
...
async def ekle_price(update, context):
    try: price = float(update.message.text.strip().replace(",",".").replace("₺",""))
    except:
    ...
    PRODUCTS.append(product); save_products(PRODUCTS); context.user_data.clear()
    await update.message.reply_text(
...
async def cmd_liste(update, context):
    if not PRODUCTS:
        await update.message.reply_text("📭 Liste boş. /ekle ile takip ekle."); return
    type_icons = {"link":"🔗","keyword":"🔍","seller":"🏪"}
    ...
    for p in PRODUCTS:
        ...
        pa    = p.get("paused_until")
        if pa and time.time()<pa: durum=f"⏸{int((pa-time.time())/60)}dk"
        label = p.get("keyword") or p.get("seller_name") or p.get("name","?")
        cur   = current_price(p)
        cur_str = f" | {fmt(cur)}" if cur else ""
...
async def cmd_sil(update, context):
    if not PRODUCTS:
        await update.message.reply_text("📭 Liste boş."); return
    type_icons = {"link":"🔗","keyword":"🔍","seller":"🏪"}
    kb = InlineKeyboardMarkup([
...
async def cmd_fiyat(update, context):
    ...
    if not links:
        await update.message.reply_text("📭 Aktif link takibi yok."); return
    kb = InlineKeyboardMarkup([
...
async def cmd_en_ucuz(update, context):
    ...
    if not aktif:
        await update.message.reply_text("📭 Veri yok."); return
...
async def cmd_baslatAll(update, context):
    ...
    for p in PRODUCTS: p["active"]=True; p["paused_until"]=None
    save_products(PRODUCTS)
...
async def ayar_val_gir(update, context):
    ...
    try:
        if key in ["check_every","alert_cooldown","max_keyword_results"]: SETTINGS[key]=int(val)
        elif key=="silent_mode": SETTINGS[key]=val.lower() in("true","evet","1","açık")
        else: SETTINGS[key]=val
        save_settings(SETTINGS)
        context.user_data.clear()
        await update.message.reply_text(f"✅ <b>{key}</b> → <b>{SETTINGS[key]}</b>", parse_mode="HTML")
    except: await update.message.reply_text("❌ Geçersiz değer.")
    return ConversationHandler.END
...
async def callback_handler(update, context):
    query = update.callback_query; await query.answer()
    data  = query.data; parts = data.split("_"); action = parts[0]

    if action == "fiyatsor":
        p = next((x for x in PRODUCTS if x["id"]==parts[1]),None)
        if not p: await query.edit_message_text("❌ Bulunamadı."); return
        await query.edit_message_text(f"🔍 <b>{p['name']}</b> sorgulanıyor...", parse_mode="HTML")
        price, in_stock = fetch_price_link(p)
        if price is None: await query.edit_message_text("⚠️ Fiyat alınamadı.", parse_mode="HTML"); return
    ...
    if action == "history":
        ...
        if not p: await query.edit_message_text("❌ Bulunamadı."); return
        h = p.get("price_history",[])
        if not h: await query.edit_message_text("📊 Veri yok."); return
        lines = [f"📊 <b>{p.get('name')} — Geçmiş</b>\n"]
        for e in h[-10:][::-1]: lines.append(f"  {e['date']} → {fmt(e['price'])}")
        low,high = lowest_price(p),highest_price(p)
        if low:  lines.append(f"\n🏆 En düşük: {fmt(low)}")
        if high: lines.append(f"📈 En yüksek: {fmt(high)}")
    ...
    if action=="pause":
        mins=int(parts[1]); p["paused_until"]=time.time()+mins*60
...
async def check_link(app, product):
    ...
    if price > product["target_price"]: return
    last = product.get("last_alerted")
    if last and (time.time()-last)/60 < SETTINGS.get("alert_cooldown", ALERT_COOLDOWN): return
    if is_silent(): return
    ...
    product["last_alerted"] = time.time(); save_products(PRODUCTS)
...
async def check_keyword(app, product):
    ...
    for item in results:
        if item["price"] > product["target_price"]: continue
        if item["url"] in seen: continue  # zaten bildirildi

        # Cooldown
        last = product.get("last_alerted")
        if last and (time.time()-last)/60 < SETTINGS.get("alert_cooldown", ALERT_COOLDOWN): continue
...
async def check_seller(app, product):
    ...
    for item in items:
        ...
        if not (is_new or is_cheap): continue
        if not is_new and not is_cheap: continue
...
async def run_price_check(app):
    ...
    for product in PRODUCTS:
        ...
        if pa and time.time() < pa: continue
        try:
...
def start_scheduler(app):
    ...
    async def daily():
        try: await app.bot.send_message(chat_id=CHAT_ID, text=build_daily_report(), parse_mode="HTML")
        except Exception as e: log.error(f"Rapor hata: {e}")
...
def main():
    ...
    async def on_startup(a):
        start_scheduler(a); await asyncio.sleep(3); await run_price_check(a)
